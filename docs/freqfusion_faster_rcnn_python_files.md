...
> ✅ Encontrarás copias actualizadas de todos estos archivos dentro de la carpeta [`freqfusion_faster_rcnn_bundle/`](../freqfusion_faster_rcnn_bundle/), donde se incluye un **subconjunto curado de `mmdet`** con los módulos necesarios para Faster R-CNN + FreqFusion.
...
| `mmdet/models/dense_heads/rpn_head.py` | Cabecera de la RPN. |
...
> Nota: El paquete incluye un subconjunto de MMDetection v2.28.1 con las extensiones de FreqFusion ya integradas. Únicamente necesitas instalar las dependencias de Python (MMCV, MMEngine, etc.) para ejecutarlo.
